        ...
        new Promise(function(resolve) {
            ...
            socket.on('disconnect', resolve)
                ...
                .on('input.touchMoveIos', function(channel, data) {
                    trySendPush([
                        channel,
                        wireutil.envelope(new wire.TouchMoveIosMessage(data.toX, data.toY, data.fromX, data.fromY, data.duration || 0))
                    ])
                })
                ...
                // Key events
                .on('input.keyDown', createKeyHandler(KeyDownMessage))
                .on('input.keyUp', createKeyHandler(KeyUpMessage))
                .on('input.keyPress', createKeyHandler(KeyPressMessage))
